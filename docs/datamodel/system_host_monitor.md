...
| 数据库                    | opsRobot                                                                                                                      |
| 数据表                    | 1. hostmetricssum：累计型主机指标数据表2. hostmetricsgauge：瞬时型主机指标数据表                                                                    |
...
hostmetricssum 与 hostmetricsgauge 两张数据表的表结构完全一致，仅存储的指标聚合类型不同，以下为完整的字段定义说明。
...
使用jsonextract函数提取深层级 JSON 字段，结合维度过滤查询指定磁盘分区的读写操作次数：
