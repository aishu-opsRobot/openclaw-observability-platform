...
export default function MonitorLeftColumn() {
  return (
    <div className="flex flex-col gap-4 w-full lg:w-1/4 h-full">
      <MonitorPanel title="每日 Token 消耗" className="flex-[4] min-h-[180px]">
        <ReactECharts option={getDailyTokenOption()} style={{ height: "100%", width: "100%" }} />
      </MonitorPanel>

      <MonitorPanel title="数字员工列表" className="flex-[6] min-h-[200px]">
            ...
            {DIGITAL_EMPLOYEE_ROWS.map((agent, i) => (
              ...
              >
                ...
                <div className="w-[20%] flex justify-center items-center gap-1">
                  ...
                  />
                  <span className={agent.status === "在线" ? "text-[#00f0ff]" : "text-[#6b7280]"}>
                    {agent.status}
                  </span>
                </div>
